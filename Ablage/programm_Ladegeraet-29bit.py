...
class Ladegeraet(threading.Thread):
    def __init__(self, debugOutput, updateCycle, outCANfromLadegeraet, inCANtoLadegeraet):
        ...
        self.readcan = can.interface.Bus(channel = 'can0', bustype = 'socketcan', bitrate=500000, can_filters = [{"can_id": 0x18FF50E5 , "can_mask": 0x1FFFFFFF, "extended": True}])
    ...
    def writeLadegeraet(self, data):
        ...
        data_Ladegeraet =int(self.Ladeschlussspannung).to_bytes(2, 'big') + int(self.Ladestrom).to_bytes(2, 'big') + int(self.Regler).to_bytes(1, 'big') + int(self.SoC).to_bytes(1, 'big') + int(0).to_bytes(2, 'big')
    ...
    def run(self):
        ...
        while(self.running):
            ...
            try:
                ...
                #print('MSG',msg)
                try:
                    if str(hex(msg.arbitration_id))=='0x18ff50e5':  # pruefe auf ladegerate adresse nur Ladegerate id
