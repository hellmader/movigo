...
HPUP_PROTOCOL_VERSION: Final[int] = 3  # Hellpower UDP protocol version
STRUCT_FORMAT_HEADER: Final[str] = "!HB2H?H"
STRUCT_FORMAT_INPUTDATA_PAYLOAD: Final[str] = "13f2H4f2H2fH15f6B"
STRUCT_FORMAT_OUTPUTDATA_PAYLOAD: Final[str] = "3fB"
HPUP_FRAMETYPE_REQUEST: Final[int] = 0x01
HPUP_FRAMETYPE_RESPONSE: Final[int] = 0x02

SIZE_STANDARD_HEADERS: Final[int] = 42  # Ethernet, IP, UDP
...
def _limit_float(inputvalue: float, minvalue: float, maxvalue: float) -> float:
...
def _generate_bit_description_table(title: str, bitdescriptions: Dict[int, str]) -> str:
...
class Inputdata:
    ...
    temperature_5: float = 0.0
    temperature_6: float = 0.0
    temperature_7: float = 0.0
    temperature_8: float = 0.0
    ...
    def __str__(self) -> str:
        ...
        return (
            ...
            + f"T5: {self.temperature_5:.1f}C T6: {self.temperature_6:.1f}C T7: {self.temperature_7:.1f}C T8: {self.temperature_8:.1f}C\n"
            + f"   Allowed {self.allowed_cell_voltage_min:.2f}V to {self.allowed_cell_voltage_max:.2f}V "
            + f"Charge max {self.allowed_charge_current:.2f}A {self.allowed_charge_temp_min:.1f}C to {self.allowed_charge_temp_max:.1f}C "
            + f"Disharge max {self.allowed_discharge_current:.2f}A {self.allowed_discharge_temp_min:.1f}C to {self.allowed_discharge_temp_max:.1f}C\n"
    ...
    def pack(self) -> bytes:
        ...
        return self.formatter.pack(
            ...
            float(self.temperature_5),
            float(self.temperature_6),
            float(self.temperature_7),
            float(self.temperature_8),
    ...
    def describe_protocol(cls) -> str:
        ...
        * Frame size on the wire (including Ethernet, IP and UDP headers): { SIZE_STANDARD_HEADERS + SIZE_INPUTDATA_FRAME}
        * UDP payload size: { SIZE_INPUTDATA_FRAME } bytes.
        * Payload size within HPUP: { SIZE_INPUTDATA_PAYLOAD } bytes
...
class Outputdata:
    ...
    # Int is uint16_t
    # Fields named 'bits' are uint8_t on the wire
    commanded_chargerate: float = 0.0  # Range 0.0-1.0, see below
    fa1: float = 0.0
    fa2: float = 0.0
    controlbits: int = 0
    ...
    def __str__(self) -> str:
        ...
        return f"{self.__class__.__name__} {string_valid} Commanded relative charge rate {self.commanded_chargerate:.2f} Bits: {self.controlbits:#04x} = {string_controlbits}"
    ...
    def pack(self) -> bytes:
        ...
        return self.formatter.pack(
            ...
            bool(self.valid),
            int(self.payload_size),
            float(self.commanded_chargerate),
            float(self.fa1),
            float(self.fa2),
    ...
    def unpack(cls, data: bytes) -> Outputdata:
        ...
        return cls(
            _limit_float(commanded_chargerate, 0.0, 1.0),
            fa1,
            fa2,
            controlbits,
            valid,
    ...
    def describe_protocol(cls) -> str:
        ...
        * Frame size on the wire (including Ethernet, IP and UDP headers): { SIZE_STANDARD_HEADERS + SIZE_OUTPUTDATA_FRAME}
        * UDP payload size: { SIZE_OUTPUTDATA_FRAME } bytes.
        * Payload size within HPUP: { SIZE_OUTPUTDATA_PAYLOAD } bytes
