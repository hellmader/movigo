...
def main() -> None:
    ...
    output.commanded_chargerate = 0.65

    while True:
        if data_age_valid:
            if time.time() - receive_timestamp > TIMEOUT_VALUE:
                print(
                    "  *** Data is now invalid due to timeout (No data from battery pack client) ***"
                )
                data_age_valid = False
        ...
        if not data_age_valid:
            print("  *** Data validity is now controlled by battery pack client ***")
        data_age_valid = True
        ...
        output.commanded_chargerate += 0.05
        if output.commanded_chargerate > 1.4:
            output.commanded_chargerate = -0.4
